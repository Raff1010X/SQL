...
echo "$($PSQL "SELECT course FROM students FULL JOIN majors USING(major_id) FULL JOIN majors_courses USING(major_id) FULL JOIN courses USING(course_id) GROUP BY course_id HAVING COUNT(student_id) = 1 ORDER BY course")"
...
#select first_name, last_name, major, gpa from students left join majors on students.major_id = majors.major_id where major = 'Data Science' or gpa >= 3.8 order by gpa desc limit 5;
